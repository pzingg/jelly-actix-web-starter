...
impl PasswordField {
    pub fn validate_with(&mut self, user_inputs: &[&str], cfg: &PasswordConfig) -> bool {
        let length_valid =
            match &cfg.length {
                Some(length) => self.validate_length(&length),
                _ => true,
            };
        let regex_valid =
            match &cfg.regex {
    ...
    pub fn validate_length(&mut self, length: &Range<usize>) -> bool {
        match self.value.len() {
            ...
            x if x < length.start => {
                self.errors.push(format!("Password must be at least {} characters.", length.start));
                false
            },
            x if x >= length.end => {
                self.errors.push(format!("Password must be at most {} characters.", length.end-1));
                false
            },
            _ => true,
        }
...
lazy_static! {
    pub static ref DEFAULT_REGEX: RegexConfig =
        RegexConfig::new(
...
impl Default for PasswordConfig {
    fn default() -> Self {
        PasswordConfig { length: Some(8..256), regex: Some(DEFAULT_REGEX.clone()), strength: None }
    }
}
